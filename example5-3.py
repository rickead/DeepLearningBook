...
def process_command_line(argv):
    ...
    # define options here:
    parser.add_option("--train", dest="train_model", default=False, action='store_true', help="Train and test model. Default is test model only.")
...
def train_model(checkpoint_dir):
    ...
    embedding_dim = 256
    rnn_units=1024
    model = build_gru_model(vocab_size=vocab_size, embedding_dim=embedding_dim, rnn_units=rnn_units, batch_size=BATCH_SIZE)
    ...
    model.compile(optimize='adam', loss=loss)
...
def test_model(saved_model_path):
    ...
    model = build_gru_model(vocab_size=vocab_size, embedding_dim=embedding_dim, rnn_units=rnn_units, batch_size=1)
